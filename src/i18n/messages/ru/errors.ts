...
const errors = {
  ...
  // Якорь комментария (привязка к фрагменту). 422-коды бека, см.
  // POST /api/lectures/{id}/comments. ANCHOR_BLOCK_NOT_FOUND — частый кейс при
  // дрейфе текста выделения.
  ANCHOR_ENTITY_UNKNOWN: "Неизвестный тип цели для привязки комментария.",
  ANCHOR_BLOCK_NOT_FOUND: "Выделенный фрагмент больше недоступен — выделите заново.",
  ANCHOR_TARGET_NOT_FOUND: "Цель привязки комментария не найдена.",
  ANCHOR_TARGET_WRONG_LECTURE: "Цель привязки относится к другой лекции.",
  ...
  // --- api-error: 413 — общий дефолт (создание комментария, поиск/контекст и др.) ---
  ...
  // --- tokens slice: доменные коды ---
  TOKEN_LIMIT: "Достигнут лимит токенов. Отзовите неиспользуемые и повторите.",
