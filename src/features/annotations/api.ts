 ...
 * Бэк клампит limit листингов аннотаций server-side (подтверждено: default 20,
 * max 100 — limit>100 молча зажимается до 100). Поэтому: (а) `< limit`-предикат
 * оборвал бы обход после 1-й страницы, МОЛЧА потеряв >100; (б) шаг offset на
 * запрошенный limit ПЕРЕПРЫГНУЛ бы записи. Шаг по items.length + терминация по
 * пустой странице корректны независимо от клампа. `limit=100` — серверный потолок
 * и оптимум страницы. `maxPages` — защита от бесконечного цикла на битом бэке.
 */
