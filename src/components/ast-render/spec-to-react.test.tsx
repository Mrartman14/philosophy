...
describe("specToReact + NODE_MAP/MARK_MAP (срез спайка)", () => {
  ...
  it("heading без level → дефолт <h2>", () => {
    const spec = NODE_MAP.heading!({ type: "heading", attrs: { blockId: "h2" } });
  ...
  it("glossary_ref с пустым id → null (голый текст в read)", () => {
    expect(MARK_MAP.glossary_ref!({ type: "glossary_ref", attrs: { id: "" } })).toBeNull();
  });

  it("bold mark → <strong>", () => {
    const m = MARK_MAP.bold!({ type: "bold" })!;
