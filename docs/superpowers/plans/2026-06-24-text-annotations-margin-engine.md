...
- [ ] **Step 3a: `ast-render.tsx` — `data-ast-root`**

```tsx
// в src/components/ast-render/ast-render.tsx — на обёртке:
return (
  <div className="ast-render" data-ast-root>
    {blocks.map((block, i) => (
...
2. Контент-рут уже несёт `data-ast-root` (Task 7, внутри `AstRender`) — **дополнительная обёртка не нужна**; связка ищет `[data-ast-root]`.
