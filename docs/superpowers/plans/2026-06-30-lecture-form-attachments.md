...
- [ ] **Step 2: lectures.ts — `searchFormPlaceholder` рядом с `searchMediaPlaceholder` и `loadFormsFailed` рядом с `loadMediaFailed` (внутри `api`), во ВСЕХ 4 локалях**

ru: `searchFormPlaceholder: "Поиск формы…",` и в блоке `api`: `loadFormsFailed: "Не удалось загрузить формы лекции",`
en: `searchFormPlaceholder: "Search form…",` / `loadFormsFailed: "Failed to load lecture forms",`
ar: `searchFormPlaceholder: "ابحث عن نموذج…",` / `loadFormsFailed: "تعذّر تحميل نماذج المحاضرة",`
zh: `searchFormPlaceholder: "搜索表单…",` / `loadFormsFailed: "无法加载讲座表单",`
...
Источник пикера — `/api/me/forms` (формы владельца). У эндпоинта нет `q`/пагинации → FE-стопгап: фильтр по `title` подстрокой + срез `offset/limit` в action. Возвращает `{ data: {id,label}[], total }` — контракт `AttachTargetPicker`.
...
Флаг бэку (из спеки): `/api/me/forms` без `q`/пагинации — желателен `GET /api/forms` (picker) с `q`/`offset`/`limit` для единообразия с документами/медиа; пока работает FE-стопгап.
