...
function Tab({ value, children }: TabProps) {
  return (
    <BaseTabs.Tab
      value={value}
      className={cn(
        "-mb-px cursor-pointer rounded-t border-b-2 border-transparent px-3 py-1.5 text-sm",
...
git commit -m "feat(ui-kit): Tabs-примитив на Base UI Tabs"
```
...
- Удаляет (namespace `admin`): `attachmentsMetaTitle`, `attachmentsPageTitle` (страница attachments удаляется в Task 4). Оставить: `attachmentsDocumentFallback`, `attachmentsDocsSectionTitle`, `attachmentsMediaSectionTitle` (используются менеджерами на карточке).

- [ ] **Step 1: admin — добавить ключи и удалить мёртвые (все 4 локали)**

В каждом `…/admin.ts` рядом с блоком `attachments*` добавить ключи и удалить `attachmentsMetaTitle` + `attachmentsPageTitle`.
...
```
В каждом файле найти и **удалить** строки `attachmentsMetaTitle: …` и `attachmentsPageTitle: …`.
...
Expected: PASS — все локали имеют одинаковый набор ключей (паритет), компиляция каталогов ок. Если паритет-тест ругается на оставшийся `attachmentsMetaTitle`/`attachmentsPageTitle` где-то — удалить и там.
...
git commit -m "i18n(lectures): ключи карточки лекции (admin+lectures, ru/en/ar/zh)"
```
...
function LazyDocPanel({
  ...
  if (state.status === "loading") {
    return <p className="text-sm text-(--color-fg-muted)">{tL("docTabLoading")}</p>;
  }
  if (state.status === "error") {
    return <p className="text-sm text-(--color-danger-fg)">{tL("docTabError")}</p>;
  }
  return (
    <article className="content">
      {state.blocks.length === 0 ? (
        <p className="text-sm text-(--color-fg-muted)">{tL("docTabEmpty")}</p>
      ) : (
...
git commit -m "feat(lectures): LectureDocumentTabs — вкладки документов с ленивой загрузкой тела"
```
...
export default async function LectureCardPage({ params }: Props) {
  ...
  async function loadDocBlocks(docId: string): Promise<ActionResult<AstBlock[]>> {
    ...
    try {
      ...
      return { success: true, data: (doc?.blocks ?? []) as AstBlock[] };
    } catch (e) {
  ...
  // — рендер медиа — тянем полные записи (гарантированный url; список может его не нести) —
  const mediaFull = (await Promise.all(media.map((m) => getMediaById(m.id)))).filter(
...
- [ ] **Step 2: Удалить attachments-страницу**
...
Expected: PASS. Линт не должен ругаться на cross-feature (импорты `@/features/*` — со страницы, разрешены). Если TS ругается на присваивание `doc.blocks` → `AstBlock[]` — это **не должно** случиться (так же делает `DocumentDetail`); при ошибке проверить, что тип `AstBlock` совместим, иначе `as AstBlock[]` уже стоит.
...
```bash
git add "src/app/admin/lectures/[id]/page.tsx"
git commit -m "feat(lectures): карточка /admin/lectures/[id] — управление + рендер вложений; удалить attachments-страницу"
```
...
```bash
git add "src/app/admin/lectures/[id]/edit/page.tsx" src/features/lectures/ui/lecture-admin-row.tsx
git commit -m "feat(lectures): ссылки на карточку лекции (edit-страница + admin-строка)"
```
...
- **`url` в списке `GET /api/lectures/{id}/media`** — проверить на лекции С медиа: если список несёт `url`, можно убрать per-item `getMediaById` в Task 4 и брать `m.url` из списка (упрощение). Если не несёт — это ожидаемо, оставляем `getMediaById`.
