...
### Task 3: Проводка в `MediaPlayer`

Повесить `PipButton` в video-ветку плеера. Расширить интеграционный тест плеера. (i18n-ключи `pipEnter`/`pipExit` УЖЕ добавлены в Task 2 — PipButton их зовёт; здесь НЕ дублировать.)
...
- [ ] **Step 1: Подтвердить наличие i18n-ключей (добавлены в Task 2)**

Run: `grep -nE "pipEnter|pipExit" src/i18n/messages/ru/media.ts src/i18n/messages/en/media.ts`
Expected: оба ключа присутствуют в ОБОИХ каталогах (ru и en). Если их нет — STOP, сообщить контроллеру (нарушен порядок тасков). НЕ добавлять ключи повторно (будет дубль-property).
...
```bash
git add src/features/media/ui/media-player.tsx src/features/media/ui/media-player.test.tsx
git commit -m "$(cat <<'EOF'
feat(media): подключить PipButton к video в MediaPlayer

Video-ветка плеера рендерит PipButton под <video> (i18n-ключи
pipEnter/pipExit уже добавлены в Task 2). Аудио-ветка не тронута.
