...
export function cellGridPos(cell: Element): { row: number; col: number } | null {
  ...
  const row = Array.prototype.indexOf.call(table.querySelectorAll("tr"), tr);
...
it("resolveAnchor: мёртвый угол (node_id нет) → null", () => {
  const r = root('<table data-block-id="t1"><tbody><tr><td data-node-id="c1">aa</td></tr></tbody></table>');
  const g = resolveAnchor(
...
it("resolveAnchor: линейный within-leaf → kind:range", () => {
  const r = root('<p data-block-id="p1" data-node-id="p1">Hello</p>');
  const g = resolveAnchor(
...
> `root(html)` — хелпер уже есть в файле (из Core). Заменить `!` на `must()`/`?.` перед коммитом.
...
(Хуки `useAnchorHighlights`/`useHoverReveal`/`useTextClick` пока получают `ranges` — без изменений в этой задаче. `overlayRanges`/overlay — Task 5.)
...
Expected: PASS, lint clean. Обновить hover/click-тесты, если они передавали `ranges` (теперь `geometries`). Прогнать `pnpm exec vitest run src/components/anchor-engine`.
...
- **Placeholders:** Шаги T4 Step1 и T5 Step1 содержат тест-ЗАГЛУШКИ (`expect(true).toBe(true)`) с явной пометкой заменить реальным драйвером по образцу существующего теста файла — потому что точный driver (renderHook/монтаж слоя) зависит от существующего сетапа файла, которого нет в этом плане дословно. Это НЕ требование-плейсхолдер, а указание на конкретный существующий источник паттерна. Имплементер ОБЯЗАН заменить заглушку реальными ассертами (форма ассерта задана в комментарии).
