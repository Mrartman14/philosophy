...
- [ ] **Step 1: Скопировать шаблон и убрать README шаблона**

```bash
cp -R src/features/_template src/features/statistics
rm src/features/statistics/README.md
```
...
(`README.md` шаблона удалён предыдущим шагом — в слайсе его быть не должно.)
...
> Примечание: тест выключения открывает Base UI `Dialog` (портал в `document.body`) и
> ищет кнопку подтверждения через `findByRole`. Если в jsdom портал/анимация Base UI
> окажутся капризными — допустимо как фолбэк проверить, что при `initialEnabled: true`
> рендерится триггер «Выключить» (см. тест выше), а логику `apply(false)` считать
> покрытой симметрично `apply(true)`. Но основной вариант — полный флоу диалога.
...
Expected: PASS — все тесты слайса (permissions 3, schemas 2, production-table 3, view-stats 3, toggle 6).

Примечание по error-boundary: fetchers (`getProductionStats`/`getViewStats`/`getHistorySettings`) кидают при ошибке бэка; их ловит существующий `src/app/error.tsx` (уже есть в проекте — отдельно создавать не нужно). Для `/me/stats` этого глобального boundary достаточно.
...
## Правки по итогам мульти-агентного ревью (2026-06-15)

Применено в план:
- `fmt`: убрана недостижимая ветка `n === null` (поля schema — `number | undefined`),
  иначе `@typescript-eslint/no-unnecessary-condition` (error в strictTypeChecked) роняет `pnpm lint`.
- `view-stats.test.tsx`: добавлен `vi.mock("next/link", …)` — конвенция изоляции юнита
  (как `router-link.test.tsx`); плюс усилены ассерты (total, `count_by_type`, фоллбек «Без названия»).
- `history-tracking-toggle.test.tsx`: убран `@testing-library/user-event` (его НЕТ в deps,
  `package.json` — запретная зона) → `fireEvent`; добавлен тест ветки ВЫКЛЮЧЕНИЯ через ConfirmDialog
  (критичная необратимая операция).
- Тоггл: `confirmLabel` изменён на «Удалить историю» — устраняет коллизию с триггером «Выключить»
  (две одинаковые кнопки) и усиливает предупреждение об удалении.
- Task 1: удаление `README.md` шаблона после `cp -R`.
- Task 7: React-`key` с фолбэком на индекс (`entity_type` опционален в schema).
- Task 13: явная отметка, что throw'ы fetcher'ов ловит существующий `src/app/error.tsx`.

Рассмотрено и сознательно НЕ менялось:
- `LABELS` дублируется в двух компонентах — оставлено намеренно: это РАЗНЫЕ словари
  (production = 8 типов, view = 5), общий объект пришлось бы класть в `@/utils/*` (запретная зона);
  YAGNI > shared-абстракция (консенсус ревьюеров).
- `revalidateEntity` не вызывается в action — для пер-юзерных fetcher'ов на `React.cache`
  (без тегов) это no-op; синхронизация через `router.refresh()` — валидный паттерн проекта.
- Гейт страниц `if (!me) redirect(...)` — оставлен (совпадает с текущим `/settings`; GET-эндпоинты
  `/api/me/*` гейтятся только аутентификацией, suspended может читать свою статистику).
