...
**Дата:** 2026-06-24
**Статус:** РЕАЛИЗОВАНО (worktree ast-heading-navigation, 7 задач + post-audit fix).

> **Ревизия post-audit (2026-06-24):** мульти-агентный аудит кода выявил
> латентную коллизию dup-id: индекс-фолбэк `heading-${index}` у заголовка,
> вложенного в blockquote/list (локальный индекс), мог совпасть с top-level
> фолбэком (битый якорь, невалидный HTML). Решение: **индекс-фолбэк УБРАН.
> DOM-id заголовка = ТОЛЬКО backend-owned `block.id` (UUID).** Заголовок без
> `block.id` (нарушение контракта бэка) не получает DOM-id и не попадает в
> оглавление (рендер и `extractHeadings` опускают его симметрично). Это
> отменяет описанный ниже «детерминированный фолбэк по индексу» (§1/§3/§граничные
> случаи/§открытые вопросы) — `headingDomId` удалён, обвязка `index` в
> `BlockRenderer` снята. Бэк-аск усилён: FE **требует** `block.id` на каждом
> heading-блоке (раньше — мягкая деградация).
