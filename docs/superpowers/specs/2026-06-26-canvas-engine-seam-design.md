...
- Добавляем:
  ...
  - `hitTestEdge(p, edges, nodesById, tol) → edgeId | null` — расстояние от точки
    до отрезка ребра; `tol ≈ 6` мировых единиц (половина нынешней прозрачной
    «широкой» подложки-`<path>` шириной 12).

  Допуски — **константы в мировых единицах** (НЕ `1/zoom`): нынешние SVG-хит-зоны
  живут внутри `viewBox`, т.е. в мировых координатах и масштабируются с зумом.
  Константный мировой допуск точно воспроизводит это (что видно — то и
  кликабельно). Точка `p` приходит уже в мировых координатах (через
  `screenToWorld`), так что зум учитывать не нужно.
...
- **R1 — пороги hit-test** (рёбра трудно кликнуть / порты мелкие на крайних
  зумах). Митигация: допуски — константы в мировых единицах, повторяющие текущие
  SVG-хит-зоны (внутри `viewBox`); юнит-тесты фиксируют математику; финальная
  подгонка в QA.
