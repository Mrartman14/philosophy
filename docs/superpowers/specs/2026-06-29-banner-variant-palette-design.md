...
10. **Флип политики — СДЕЛАНО** (`src/security/csp.ts`): в проде `style-src 'self'
    'nonce-…'` (убран `'unsafe-inline'`), `style-src-elem 'self' 'nonce-…'`,
    `style-src-attr 'none'`. Прод-политика теперь несёт **ноль `'unsafe-inline'`**.
    dev оставляет `'unsafe-inline'` (HMR). TDD в csp.test.ts. Политика по-прежнему
    Report-Only до `CSP_ENFORCE=1`.
11. **ESLint-гард — ОТКЛОНЁН (осознанно).** Blanket-запрет `style={}` некорректен:
    flat-config ESLint скоупит по glob, а не по server/client, при этом клиентские
    inline-стили (React/floating-ui) идут через CSSOM и под CSP НЕ попадают — ban дал
    бы сотни ложных срабатываний. Регрессии ловит **CSP Report-Only**: он наблюдает
    реальные нарушения в SSR-разметке и сам отличает их от CSSOM-exempt клиентских
    стилей (lint-эвристика не может). Узкий file-scoped гард на конкретные SSR-файлы
    возможен, но произволен/хрупок и избыточен поверх CSP-репортов.
