...
### Открытый бэк-аск (не блокер v1)

`comment.Service.validateAnchor` (`internal/comment/service.go:1092`) проверяет
существование блоков в целевой сущности, но НЕ проверяет, что цель принадлежит
той же лекции, что и комментарий. Теоретически комментарий лекции A может
заякориться на блок документа лекции B. FE всегда якорится на инлайн-документ
текущей лекции, так что нас не блокирует. Вопрос владельцу бэка: это намеренно
(cross-lecture anchoring разрешён) или нужна инвариант-проверка с кодом
`PARENT_WRONG_LECTURE` (уже есть в `apperror.Code`)?
