...
1. **Структурный маркер.** Обёртка `ast-render` несёт `data-ast-root` (аддитивно, foundation).
   Движок принимает `astRootRef`, указывающий на `[data-ast-root]`; в dev — `log.warn`, если рут
   не помечен (защита от подключения к не-AST поверхности).
2. **Валидный захват = обе границы в AST-блоках одного рута.** `anchorFromRange` возвращает
   `TextAnchor` ТОЛЬКО если и start, и end резолвятся в `closest('[data-block-id]')` с непустым id
   и оба внутри `astRootRef`. Выделение, начавшееся в AST и ушедшее в карточку/сайдбар/не-AST —
   → `null`. Аффорданс «+ Аннотация» показывается лишь при не-`null` якоре → кнопка физически
   не появляется на не-AST тексте (это и есть устрожение, не «тихий отказ»).
