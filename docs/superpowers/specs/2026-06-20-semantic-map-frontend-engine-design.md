...
**Дизайн (brainstorming), не реализован.** Бэкенд (`/api/map`) тоже ещё не
реализован — движок пишется против фикстур контрактной формы и подключается к
реальной ручке за тем же интерфейсом данных без переделки.
