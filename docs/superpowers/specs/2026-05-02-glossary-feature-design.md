...
- `src/api/tags.ts` — добавить тег `glossary` (если ещё нет).
...
| `bullet_list`, `ordered_list`, `list_item` | `code` | |
| `code_block` | `link` | |
...
- `block-renderer.tsx` — exhaustive switch с `block satisfies never` в default. Появление нового типа в схеме → TS-ошибка.
- Round-trip тесты: фикстуры из `__fixtures__/blocks.ts` рендерятся через `AstRender` → snapshot. Параллельно тот же AST дёргается через существующий `AstEditor` deserializer + ProseMirror-DOM в отдельном тесте, и DOM-семантика (теги, текст, href'ы) сравнивается с `AstRender` через `data-testid`-якоря. Drift = красный тест.
...
export const TermBlocksUpdateSchema = z.object({
  ...
  blocks: z.string().min(1, "Тело не может быть пустым"),
});
...
`blocks` приходит из формы как сериализованный JSON (паттерн `AstEditor` в `lectures`). Тяжёлую AST-валидацию делает бэк — фронт в schema проверяет только «не пустая строка».
...
export const updateTermBlocks = createFormAction(async (formData) => {
  ...
  const blocks = JSON.parse(input.blocks); // throw на битом JSON, ловится createFormAction
  const api = await createApiClient();
...
| `glossary-edit-form.tsx` | Обёртка `<AstEditor defaultValue={blocks}>` + submit | client |
...
- **Точная форма пустого `paragraph`-stub** для `createTerm` — определяется при реализации (фикстура + Go-модель). Если бэк строго валидирует структуру paragraph при create, может потребоваться передавать `text: ""` или конкретные поля.
- **Сортировка list-эндпойнта** — формально не зафиксирована в OpenAPI; решено всегда сортировать на фронте перед рендером (см. §6.1).
- **Drift `AstRender` с `AstEditor`** — закрывается round-trip тестами, но в первой версии набор поддерживаемых block-типов меньше, чем в редакторе. Фоллбек на plain text безопасен, но в dev-режиме `console.warn` поможет ловить регрессии раньше тестов.
- **Размер бандла на edit-странице.** `AstEditor` уже грузится для лекций; повторное использование на `/admin/glossary/[id]/edit` не добавит новых тяжёлых зависимостей.
