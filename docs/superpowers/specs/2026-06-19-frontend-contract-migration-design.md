...
## Уточнения после выверки кода (2026-06-19)

Часть «foundation» уже существует — план это учитывает (не переписываем готовое):

- **Optlock уже на 7 из 9 PUT-ручек** (documents, comments, annotations, banners, events, glossary — общий хелпер `ifMatchHeader(formData, …)` читает числовой `version` из hidden-поля; canvas — сырой ETag-заголовок). **Добавить нужно только `lectures` и `trails`.** Отдельный `src/utils/optlock.ts` НЕ нужен — расширяем существующий паттерн `ifMatchHeader`.
- **Канал ошибок по полям уже есть**: `ActionResult` имеет `code:"validation"; fieldErrors`; `parseFormData`→`ZodValidationError`→`toResult`→`<Form errors>`/`FormFeedback` уже работают для клиентского Zod. НОВОЕ — только мост серверного 422: добавить `fields?` в интерфейс `ApiError` и в `rethrowApiError` бросать `ZodValidationError(err.fields)`.
- **`loginAction` уже читает `data.access_token`** (логин по полю не сломан). Доделать: сохранять `refresh_token`, выставлять access-cookie `maxAge` из `expires_in`. `logoutAction` сейчас шлёт `Authorization: Bearer` без тела — перевести на тело `{refresh_token}`.
- **Структура планов:** Plan A (foundation: auth+refresh+middleware, мост 422, Visibility) → Plan B (слайсы: lectures optlock, trail→документы+optlock, подписка на лекции, logout-all UI) → Plan C (манифесты).
