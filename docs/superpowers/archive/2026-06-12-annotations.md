...
> **Статус: ВЫПОЛНЕНО** (ветка `feat/annotations`). Все задачи 1–19 выполнены В ОДНОЙ ВЕТКЕ: documents/media/comments уже были смержены в `main` к старту, поэтому интеграции 17 (документ) и 18 (медиа) выполнены прямо в ветке, а не follow-up'ами. Задача 19 (аннотации на комментариях): поддержка `comment` как parent-типа заложена в api/actions/типах/Zod; UI-точка монтирования под комментарием остаётся за слайсом comments (slot-проп, cross-feature-граница не нарушается). Якорь из UI в MVP не задаётся (риск §3). Gate `npm run lint && npm test && npm run build` — зелёный.
...
- [x] **Step 2: Добавить тег `ANNOTATIONS` (append-only, алфавитно)**
...
- [x] **Step 3: Проверить, что ничего не сломалось**
...
- [x] **Step 4: Запустить — должно пройти**
...
- [x] **Step 1: Поле выбора видимости (client)**
...
- [x] **Step 2: Кнопка удаления (client, ConfirmDialog)**
...
- [x] **Step 3: Проверить линт/типы/сборку роута**
...
- [x] **Step 1: Добавить fetcher в api.ts**
...
- [x] **Step 2: Компонент контекста якоря (server)**
...
- [x] **Step 2: Композитор секции (server)**
...
- [x] **Step 1: Локальная пагинация (client, паттерн audit)**
...
- [x] **Step 4: Страница «Мои аннотации»**
...
- [x] **Step 5: Страница агрегации аннотаций лекции (отдельная, НЕ трогаем page.tsx)**
...
- [x] **Step 6: Admin-страница (Layer-3 гейт)**
...
- [x] **Step 7: Проверить типы и линт**
...
- [x] **Step 2: Интегрировать секцию в страницу глоссария**
...
- [x] **Step 1:** Согласовать с владельцем `comments` slot-проп (например `<Comment renderAnnotations={(commentId) => <AnnotationsSection parentEntityType="comment" parentId={commentId} />} />`).
- [x] **Step 2:** Реализовать монтирование, проверить `npm run lint && npm run build`, commit.
...
- [x] **Step 1: Чеклист `_template/README.md`**
  - [x] `index.ts` экспортирует только нужное снаружи.
  - [x] `api.ts`, `actions.ts`, `permissions.ts`, `schemas.ts` начинаются с `import "server-only";` (anchor.ts — pure, без server-only, тестируется в jsdom).
  - [x] Каждая `canXxx` покрыта тестом (5 функций — Task 3).
  - [x] Каждая Zod-схема: success + failure (Task 5).
  - [x] Используются `createFormAction`/`createAction` + `parseFormData` + `requireCapability` + `revalidateEntity`.
  - [x] Нет импортов других `@/features/*`.
  - [x] `ui/.gitkeep` удалён, реальные UI-файлы добавлены.
...
- [x] **Step 3: Self-check бизнес-правил**
  - [x] Visibility выбирается только в create-форме, в edit-форме отсутствует (§6.8). ✓ Task 10/11.
  - [x] Admin-удаление показывается только для public (`canAdminDeleteAnnotation`). ✓ Task 3/15.
  - [x] forbidden → branded-текст «У вас нет прав на …». ✓ Task 9/10/11.
  - [x] Создание зовёт пер-сущностный POST (§10.1), список — пер-сущностный GET (§10.2). ✓ Task 6/7.
  - [x] `annotation.create` — локальный чек (frozen union). ✓ Task 3.
  - [x] `/admin/annotations` nav-пункт уже есть (annotation.delete_any) — layout не трогали. ✓.

- [x] **Step 4: Code-review-субагент** по диффу ветки (skill requesting-code-review) — bug/нарушение-конвенций фиксируются до мержа.
