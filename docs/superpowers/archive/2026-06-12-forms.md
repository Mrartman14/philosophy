...
> **For agentic workers:** REQUIRED SUB-SKILL: используй superpowers:subagent-driven-development (рекомендуется) или superpowers:executing-plans, чтобы выполнять этот план задача-за-задачей. Шаги размечены чекбоксами (`- [x]`).
>
...
- [x] **Step 1: Скопировать шаблон и переименовать**
...
- [x] **Step 2: tags.ts — добавить FORMS и SUBMISSIONS (алфавит, append-only)**
...
- [x] **Step 3: types.ts — сужения из schema.ts**
...
- [x] **Step 4: index.ts — пустой каркас (наполняется по мере задач)**
...
- [x] **Step 5: Проверить, что слайс компилируется**
...
- [x] **Step 4: Запустить тест, убедиться что зелёный**
...
- [x] **Step 5: Дополнить index.ts экспортом permissions**
...
- [x] **Step 1: Реализация actions.ts (форма: create/update/publish/delete/admin-delete)**
...
- [x] **Step 2: Не оставлять неиспользуемые импорты — объединить с Task 7**
...
- [x] **Step 1: Дописать в actions.ts действия откликов**
...
- [x] **Step 2: Дополнить index.ts всеми actions**
...
- [x] **Step 4: Commit (объединённый Task 6+7)**
...
- [x] **Step 1: form-builder-field-row.tsx (одно поле)**
...
- [x] **Step 2: form-builder.tsx (список полей + meta + скрытый payload)**
...
- [x] **Step 3: blocks-text.ts — извлечение plain-text из ast.Block[] для редактирования markdown**
...
- [x] **Step 1: form-publish-button.tsx**
...
- [x] **Step 2: form-delete-button.tsx**
...
- [x] **Step 4: src/app/me/forms/page.tsx**
...
- [x] **Step 2: src/app/admin/forms/page.tsx**
...
- [x] **Step 3: Дополнить index.ts**

```ts
export { FormAdminRow } from "./ui/form-admin-row";
```

- [x] **Step 4: Тип-чек + lint + commit**
...
- [x] **Step 1: form-meta.tsx (бейджи видимости/режима/статуса)**
...
- [x] **Step 2: form-after-submit.tsx (рендер after_submit_blocks)**
...
- [x] **Step 3: form-detail.tsx (заголовок + описание)**
...
- [x] **Step 4: index.ts + тип-чек + commit**
...
- [x] **Step 1: form-field-input.tsx (один контрол по типу поля)**
...
- [x] **Step 3: index.ts + тип-чек + lint + commit**
...
- [x] **Step 1: submission-detail.tsx (просмотр ответов отклика; нужна форма для типов полей)**
...
- [x] **Step 2: submission-edit-form.tsx (редактирование отклика на editable-форме)**
...
- [x] **Step 3: submission-actions.tsx (delete для editable / retract для immutable)**
...
- [x] **Step 4: index.ts + тип-чек + lint + commit**
...
- [x] **Step 1: submission-list.tsx (список откликов формы — для владельца)**
...
- [x] **Step 2: my-submissions-list.tsx (мои отклики)**
...
- [x] **Step 3: src/app/forms/[id]/submissions/page.tsx**
...
- [x] **Step 4: src/app/me/submissions/page.tsx**
...
- [x] **Step 5: index.ts + тип-чек + lint + commit**
...
- [x] **Step 1: Сверить index.ts — все экспорты на месте, нет дублей**
...
- [x] **Step 6: Self-review против спеки (§3 волна 3 + §6 правила 1,2,4,5,6)**

Пройди чеклист `src/features/_template/README.md` и сверь:
- [x] Видимость: UI предлагает только private→public (FormBuilder edit-режим не даёт менять visibility; publish-кнопка только для private draft). ✔ §6.1
- [x] Заморозка: `canEditForm` = false после publish; UI прячет редактирование; код ошибки `FORM_PUBLISHED` обработан branded-текстом. ✔ §6.5
- [x] Режимы: editable → edit/delete; immutable → retract; коды `FORM_IMMUTABLE_MODE`/`MODE_CHANGE_FORBIDDEN`/`RETRACT_NOT_APPLICABLE`/`ALREADY_SUBMITTED`/`ALREADY_RETRACTED` обработаны. ✔ §6.5
- [x] Приватность откликов: список — только владелец (canListFormSubmissions); edit/delete/retract — только автор; admin-override нет. ✔ §6.6
- [x] share-token: `?token=` пробрасывается в GET формы и POST submission; чтение чужих откликов через token не делается. ✔ §6.6
- [x] Аноним GET формы → notFound (бек 404). ✔
- [x] delete_any: admin удаляет только public, через обычный DELETE. ✔ §6.2
- [x] Capability-чек локальный (union не тронут); foundation-touch отмечен. ✔
- [x] 403 → branded «У вас нет прав на …». ✔ §6.10
- [x] Тесты: permissions ≥4 кейса на хелпер; schemas success+failure (режимы, заморозка через visibility-схему, choice-опции). ✔ §7

- [x] **Step 7: Commit (если были правки в self-review)**
