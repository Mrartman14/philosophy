...
### 6.2. RTL / направление письма

Вёрстка обязана быть направленно-нейтральной: одна разметка работает и в LTR, и в
RTL без форков. Это держится двумя инвариантами — **логические CSS-свойства** и
**направление из локали**.

**Логические свойства обязательны.** Вместо физических осей используй логические,
которые сами разворачиваются при `dir="rtl"`:

| Физическое (запрещено) | Логическое |
| --- | --- |
| `ml-*` / `mr-*`, `pl-*` / `pr-*` | `ms-*` / `me-*`, `ps-*` / `pe-*` |
| `left-*` / `right-*` (inset) | `start-*` / `end-*` (`inset-inline`) |
| `text-left` / `text-right` | `text-start` / `text-end` |
| `border-l` / `border-r`, `rounded-l` / `rounded-r` | `border-s` / `border-e`, `rounded-s` / `rounded-e` |
| `marginLeft` / `paddingRight` / `borderLeft` (style-объект) | `*Inline*` / `inset-inline` |

Физические direction-токены запрещены **Guardrail 10** (`eslint.config.mjs`; номер 9
занят leaf-foundation import-гардом). Гард — `no-restricted-syntax`, ловит токены в
строковых литералах (`className`, аргументы `cn()`, вынесенные const-строки классов)
и в `style={{}}`-объектах **по всему `src/**`** (вшит в последние матчащие
`no-restricted-syntax`-блоки — G6 для `ui/**`+тестов и G7/8 для прикладного кода).

**Направление — из локали, НЕ настройка.** Выводится `dirForLocale(locale)`
(`src/i18n/locales.ts`, `RTL_LOCALES = ["ar", "fa", "he", "ur"]`) и применяется
одновременно к `<html dir>` И к Base UI через `DirectionProvider`. Это **не** ось
appearance и **не** пользовательский тоггл — направление детерминировано локалью.
`DirectionProvider` обёрнут в ui-kit (`src/components/ui/direction-provider.tsx`):
прямой импорт `@base-ui/react` вне `src/components/ui/**` запрещён Guardrail 7, поэтому
kit-обёртка обязательна.

**Base UI зеркалится сам.** Через `DirectionProvider` компоненты Base UI
разворачиваются автоматически. Их `data-[side=left/right]`-компаньоны остаются
**физическими** — Base UI вычисляет `data-side` уже с учётом направления (пост-флип),
дополнительно зеркалить их не нужно.

**Спец-поверхности:**

- **3D-canvas** изолируется `dir="ltr"` на обёртке `data-scene-canvas` (вокруг canvas
  и region-labels — `src/features/semantic-map/ui/semantic-map-view.tsx`,
  `src/features/reference-graph/ui/graph-view.tsx`). Оверлей-панели остаются **снаружи**
  обёртки, чтобы зеркалиться нормально.
- **Код / идентификаторы** всегда LTR — оборачивай `dir="ltr"` / `<bdi>`: `<pre>`/`<code>`
  в ast-render (`src/components/ast-render/`), сниппеты connect-instructions
  (`src/features/tokens/ui/connect-instructions.tsx`), `token_hint` в token-list
  (`src/features/tokens/ui/token-list.tsx`).
- **Направленные стрелки навигации** — НЕ глифы в тексте лейбла. Используй `ChevronIcon`
  (`src/assets/icons/chevron-icon.tsx`) + утилиту `.rtl-flip` (`globals.css`:
  `[dir="rtl"] .rtl-flip { transform: scaleX(-1); }`). Шеврон по умолчанию смотрит в
  сторону чтения; `.rtl-flip` зеркалит его в RTL.

**Exempt-кейсы** требуют узкого `eslint-disable-next-line no-restricted-syntax -- <причина>`:
`data-[side]`-компаньоны Base UI и `collisionPadding`, центрирование диалога `left-1/2`,
инлайновые координаты canvas. Без обоснования — не отключать.

**Известные ограничения / follow-up:**

- Инструктивные `→` («затем / ведёт к») в `common.ts` (PWA iOS-подсказка) и `tokens.ts`
  (`Настройки → Connectors`) оставлены как **контент**, не навигация — bidi-алгоритм
  их разворачивает корректно.
- Глиф `→` в `.fancy-link::after` (`globals.css`) — декоративный, не флипается (minor).
- Гильметы `«»` — пунктуация, обрабатывается bidi-алгоритмом.

**Проверка RTL без реального перевода:** тесты на `dir="rtl"`
(`semantic-map-direction.test.tsx`, `globals-rtl.test.ts`) + временно
`document.documentElement.dir = "rtl"` в dev-консоли.
